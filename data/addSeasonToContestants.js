...
function processRow(row) {
  ...
  Contestant.findOne({firstName: row.firstName,lastName: row.lastName}, function(err,contestant) {
    if (err) {
      console.log(err);
      return;
    }
    if (!contestant) {
      console.log(`Contestant not found for ${row.firstName} ${row.lastName}`);
      return;
    }
    //const updatedSeasons = contestant.seasons ? contestant.seasons.push({seasonId: seasonId}) : [{seasonId: seasonId}];
    contestant.seasons ?
      contestant.seasons.push({seasonId: seasonId}) :
      contestant.seasons = [{seasonId: seasonId}];
    //contestant.seasons = updatedSeasons;
    ...
    contestant
      ...
      .catch(err => {
        if (err instanceof db.mongoose.Error.ValidationError){
          console.log(
              err.message || `Validation error occurred while adding season to contestant ${req.body.firstName} ${req.body.lastName}.`
          );
        }
        console.log(
            err.message || `Some error occurred while adding season to contestant ${req.body.firstName} ${req.body.lastName}.`
        );
      });
  });
